...
public class MerossHttpConnector {
    ...
    public String getDevUUIDByDevName(String devName) {
        try {
            return getDevices().stream().filter(device -> device.devName().equals(devName)).map(Device::uuid)
    ...
    public void fetchCredentialsAndSave(File credentialFile) {
    ...
    public void fetchDevicesAndSave(File deviceFile) {
