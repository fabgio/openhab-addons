...
public class MerossManager {
    ...
    /**
     * Initializes the mqtt connector with the properer cloud credentials
     * 
     * @throws IOException if initialization fails
     */
    public void initializeMerossMqttConnector() throws IOException {
        ...
        if (credentials == null) {
            ...
            String key = merossHttpConnector.readCredentials().key();
            MqttMessageBuilder.setKey(key);
            String brokerAddress = merossHttpConnector.readCredentials().mqttDomain();
    ...
    public void executeCommand(String deviceName, String commandType, String commandMode) throws IOException {
        ...
        if (getAbilities(deviceUUID) != null && !getAbilities(deviceUUID).isEmpty()) {
                ...
                if (allObject.has("system") && !allObject.get("system").isJsonNull()) {
                    ...
                    if (systemObject.has("online") && !systemObject.get("online").isJsonNull()) {
                        ...
                        if (!onlineObject.isEmpty()) {
                            ...
                            logger.warn("Online status missing or null");
                        }
                    }
                }
            }
    ...
    public int togglexOnOffStatus(String deviceName) throws IOException {
        String systemAll = getSystemAll(deviceName);
        if (systemAll != null && !systemAll.isEmpty()) {
            JsonObject jsonObject = JsonParser.parseString(systemAll).getAsJsonObject();
            if (jsonObject.has("payload") && !jsonObject.get("payload").isJsonNull()) {
                JsonObject payloadObject = jsonObject.getAsJsonObject("payload");
                if (payloadObject.has("all") && !payloadObject.get("all").isJsonNull()) {
                    JsonObject allObject = payloadObject.getAsJsonObject("all");
                    if (allObject.has("digest") && !allObject.get("digest").isJsonNull()) {
                        JsonObject digestObject = allObject.getAsJsonObject("digest");
                        if (digestObject.has("togglex") && !digestObject.get("togglex").isJsonNull()) {
                            JsonArray togglexArray = digestObject.getAsJsonArray("togglex");
                            if (!togglexArray.isEmpty()) {
                                Optional<JsonElement> jsonElement = Optional
                                        .of(togglexArray.get(0).getAsJsonObject().get("onoff"));
                                return jsonElement.get().getAsInt();
                            } else {
                                logger.warn("Togglex onoff status missing or null");
                            }
                        }
                    }
                }
    ...
    public boolean togglexIsOn(String deviceName) throws IOException {
        return togglexOnOffStatus(deviceName) == MerossEnum.OnOffStatus.ON.value();
    }
    ...
    public String getSystemAll(String deviceName) throws IOException {
        ...
        if (mqttResponse != null && !mqttResponse.isEmpty()) {
            return mqttResponse;
        }
        return "";
    }

    private @Nullable HashSet<String> getAbilities(String deviceName) throws IOException {
        initializeMerossMqttConnector();
        String uuid = merossHttpConnector.getDevUUIDByDevName(deviceName);
        if (uuid.isEmpty()) {
            return null;
        ...
        if (mqttResponse != null && !mqttResponse.isEmpty()) {
            digestElement = JsonParser.parseString(mqttResponse);
        }
