...
public class MerossLightHandler extends BaseThingHandler {
    ...
    public void initialize() {
        ...
        try {
            ...
            logger.error("Cannot read from file {}", e.getMessage());
            return;
        }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        var connector = MerossBridgeHandler.connector;
        if (connector == null) {
            logger.debug("MerossHttpConnector is null {}", channelUID);
            return;
        }
        var manager = MerossManager.newMerossManager(connector);
            ...
            if (command instanceof OnOffType) {
                try {
                    if (OnOffType.ON.equals(command)) {
                        manager.executeCommand(config.lightName, MerossEnum.Namespace.CONTROL_TOGGLEX.name(),
                                OnOffType.ON.name());
    ...
    private void startAutomaticRefresh() {
        scheduler.scheduleWithFixedDelay(this::updateChannel, 1, 1, TimeUnit.SECONDS);
    }
