...
public class MerossHttpConnector {
    ...
    public String getDevUUIDByDevName(String devName) {
        try {
            return readDevices().stream().filter(device -> device.devName().equals(devName)).map(Device::uuid)
                    .findFirst().orElseThrow(() -> new MerossException("No device found with name " + devName));
        } catch (MerossException e) {
            throw new RuntimeException(e);
        }
    }
     ...
     */
    public CloudCredentials raedCredentials() {
     ...
     */
    public ArrayList<Device> readDevices() {
    ...
    private String readFile(File file) {
        ...
        try {
            ...
            logger.error("Couldn't read from file '{}' exception with message {}.", file.getPath(), e.getMessage());
        }
        return content;
    }
